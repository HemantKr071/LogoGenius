  ...
  useEffect(() => {
    const GetLogos = async () => {
      ...
      if (user?.primaryEmailAddress?.emailAddress) {
        const logos = await getDocs(
          collection(db, "users", user.primaryEmailAddress.emailAddress, "logos")
        );
        const fetchedLogos = logos.docs.map((doc) => doc.data());
        console.log(fetchedLogos);

        // Set logo list directly with fetched data
        setLogoList(fetchedLogos);
      } else {
        console.log("User is not defined");
      }
    };

    // Call GetLogos if user is defined
    if (user) {
      GetLogos();
    }
  }, [user]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 px-10">
        {logoList.length > 0 ? (
          ...
          [1, 2, 3, 4, 5, 6].map((index: number) => (
            <div
              key={index}
              className="w-full bg-slate-200 animate-pulse rounded-xl h-[250px]"
            ></div>
          ))
        )}
      </div>
